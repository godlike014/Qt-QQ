    ...
    ui(new Ui::Widget)
{
    ...
    ui->sendBtn->setShortcut(QKeySequence(tr("Ctrl+Return")));
    uName = usrname;
...
void Widget::on_sendTBtn_clicked()
{
    ...
    QString usrtemp = ui->usrTblWidget->selectedItems().first()->text();
    QString iptemp = ui->usrTblWidget->selectedItems().last()->text();
    if(iptemp == getIP() && usrtemp == getUsr())
    {
        QMessageBox::warning(this,"error","不能向自己发送文件",QMessageBox::Ok);
        return;
    }
    srv->show();
    srv->initSrv();
...
{
    QString ipAddr = getIP();
    if(ipAddr == clntaddr&& (srvaddr!=clntaddr||usrname!=uName))
    {
...
void Widget::curFmtChanged(const QTextCharFormat &fmt)
{
    ...
    else ui->sizeCbx->setCurrentIndex(ui->sizeCbx->findText(QString::number(fmt.fontPointSize())));
...
void Widget::on_saveTBtn_clicked()
{
    ...
    {
        QMessageBox::warning(0,"警告",tr("聊天记录为空，无法保存！"),QMessageBox::Ok);
    }
    else
    {
